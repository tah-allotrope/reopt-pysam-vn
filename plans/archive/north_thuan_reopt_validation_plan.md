...
> **Method:** Local solver via `scripts/julia/run_vietnam_scenario.jl` + `src/julia/REoptVietnam.jl` + HiGHS. **Not the Python API path** — see rationale below.
...
Create `scripts/python/integration/compare_north_thuan_reopt_vs_staff.py`:
...
Create `scripts/python/integration/generate_north_thuan_reopt_report.py` following the same pattern as `generate_north_thuan_validation_report.py`. The report should include:
