...
> Current repo: `reopt-julia-VNanalysis`
...
- Existing `scripts/python/`, `tests/python/`, and `src/reopt_vietnam.py` already give PySAM a natural Python-side entry point.
...
- Planning documents currently live under `docs/worklog/plans/`, which makes them less visible than they should be for active iteration.
...
|---|---|---|---|
| `docs/worklog/plans/` | `plans/archive/` | Move | Historical plans remain accessible but no longer buried under docs |
| new active planning files | `plans/active/` | Create | All future iteration plans should start here |
| `src/REoptVietnam.jl` | `src/julia/REoptVietnam.jl` | Move | Keep Julia code explicit and separate from Python |
| `src/reopt_vietnam.py` | `src/python/reopt_pysam_vn/reopt/preprocess.py` | Move/refactor | Add compatibility shim during transition |
...
- Rename the local root folder from `reopt-julia-VNanalysis` to `reopt-pysam-vn`.
...
- Move existing plan files from `docs/worklog/plans/` into `plans/archive/`.
...
- Path-sensitive scripts may fail if they assume the old `src/reopt_vietnam.py` location.
...
### Q1. Should the Python layer become a proper package now? Agree with recommended 
...
### Q2. Should historical plans be moved or copied into the new `plans/` home? Agree with recommended
...
### Q3. Should historical artifacts that contain the old repo name be rewritten? Agree with recommended
...
### Q4. Which PySAM finance model should be first? N/A
...
### Q5. Should the repo rename happen before or after the code moves? Agree with recommended
...
### Q6. Should `requirements.txt` remain the primary Python environment file after PySAM is added? agree with recommended
